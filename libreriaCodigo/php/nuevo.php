...
try {
    ...
    if ($tipo === 'persona') {
        ...
        ");
        $stmt->execute([
            ':usuario_id'=>$usuarioId, ':nombre'=>$body['nombre']??'',
            ':apellido1'=>$body['apellido1']??'', ':apellido2'=>$body['apellido2']?:null,
            ':dni_nif'=>$body['dni_nif']??'', ':telefono'=>$body['telefono']??'',
            ':telefonomovil'=>$body['telefonomovil']??'', ':email'=>$body['email']??'',
            ':tipovia'=>$body['tipovia']??'', ':direccion'=>$body['direccion']??'',
            ':numero'=>$body['numero']?:null, ':portal'=>$body['portal']?:null,
            ':escalera'=>$body['escalera']?:null, ':piso'=>$body['piso']?:null,
            ':puerta'=>$body['puerta']?:null, ':cp'=>$body['cp']??'',
            ':localidad'=>$body['localidad']??'', ':municipio'=>$body['municipio']??'',
            ':provincia'=>$body['provincia']??'', ':notas'=>$body['notas']?:null,
        ]);
        ...
        ");
        $stmt->execute([
            ':usuario_id'=>$usuarioId, ':letra_provincia'=>$body['letra_provincia']??'',
            ':numero_matricula'=>$body['numero_matricula']??'', ':provincia'=>$body['provincia']??'',
            ':municipio'=>$body['municipio']?:null, ':titular_id'=>$body['titular_id']?:null,
        ...
        http_response_code(400); echo json_encode(['ok'=>false,'msg'=>'Tipo no válido']);
    }
